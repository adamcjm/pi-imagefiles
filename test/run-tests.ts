...
const result = await processPayload(makePayload([imgData]), fake, "sk-test");
...
const bigResult = await processPayload(makePayload(lots), bigFake, "sk-test");
...
failFake.fail = true;
const failResult = await processPayload(makePayload([imgData]), failFake, "sk-test");
