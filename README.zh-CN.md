...
Pi 会把每张截图以 `data:image/...;base64,...` 内联进 provider 请求。**DeepSeek 的 API 网关拒绝超过 48 MiB 的请求**（50,331,648 字节 —— `413 Request Entity Too Large`；实测：47.9 MiB 通过，48.0 MiB 失败）。一个长时间运行的视觉会话（几十张截图、每张 base64 最多 4.5 MB）最终会超过该上限，会话再也无法继续。
...
- **三方 DeepSeek 视觉网关**（如 opencode zen/go —— 它转出 `deepseek-v4-flash-vision-exp` 模型，但**没有 Files API**（`GET /v1/files` → 404），也**不接受你的 `file_id`**：它们用*自己的* API key 调 DeepSeek 上游，而 file_id 归属于上传它的 key，上游会返回 `file_ids do not exist or are not created under your account`；已实测 base64 图片可正常通过）：扩展在预算超限时裁剪*最旧*的图片，保证请求永不越过上游 48 MiB 限制。

## 工作原理

与 `dsh`（deepseek-harness）同源的流程，预算按官方文档校准：
...
3. **缓存**（仅官方）—— 以 sha256 内容寻址存于 `~/.pi/agent/data/pi-imagefiles-cache.json`；同一张图只上传一次，跨请求、跨会话复用。上传时设置 `expires_after=30 天`（文档允许的最大值），过期前 1 小时自动重新上传——既让上传保持稀少，又不会把 25 GiB / 10,000 文件的账号配额占满。
4. **预算 / 卸载**（所有模式）—— `upload` 模式下每请求 **128 MiB** 图片字节、**600 张**上限（文档规定含 `file_id` 的请求上限为 200 MiB）；`offload-only` 模式下图片保持 base64 内联，受上游 48 MiB 请求体限制，预算为 **40 MiB**；超预算时*最旧*的图片替换为带**解析图片信息**的占位文本（mime 类型、从 PNG/JPEG/WebP/GIF 头部解析的尺寸、sha256），按确定性量子（64 MiB / 20 张）裁剪。这些图片模型此前已在会话中看到并理解过，可依赖该理解。
...
已对线上 API 验证：上传 5.6 MB 截图并以 `file_id` 引用，模型能正确识别图片内容，该图在线路上仅约 200 字节。对于 opencode zen/go：base64 图片可用，2.4 MB→38.5 MB 的请求体通过，48.1 MB 被上游 413 拒绝——正是卸载裁剪要避开的同一堵墙；而 `file_id` 引用会被上游拒绝（`not created under your account`），因为网关用自己的 key 转发。
...
|---|---|
| `purpose` | `user_data` |
| 文件有效期 / 刷新余量 | 30 天 / 1 小时 |
| 每请求图片字节上限（upload 模式） | 128 MiB |
| 每请求内联字节上限（offload-only 模式） | 40 MiB |
| 每请求图片数量上限 | 600 |
| 卸载字节 / 数量量子 | 64 MiB / 20 |
| 上传超时 / 重试 | 30 秒 / 1 次 |
| 熔断 | 5 次失败 → 暂停 1 小时 |
...
- 通过本扩展上传的图片仅由 `/imagefiles reset` 从缓存删除；Files API 侧 30 天后自动过期。
